...
export default function HomeLanding({
  ...
  const realHeroCards: HeroFeature[] = allSeries.slice(0, 7).map((series) => ({
    ...
    year: series.year,
    // rating/tags omitted -- LandingPosterStage never reads them, and
    // they're optional on HeroFeature now, so no need to fabricate values.
  ...
  const realDiscoverCards: DiscoverCard[] = allSeries.slice(0, 6).map((series, i) => ({
    ...
    mediaType: 'Series',
    year: series.year,
    // Real aggregation -- null/undefined hides the rating badge instead of
    // showing a fabricated score (see DiscoverCard.rating).
    rating: series.average_rating ?? null,
    badge: DISCOVER_BADGE_ROTATION[i % DISCOVER_BADGE_ROTATION.length],
    // Real tag labels (falls back to genre names, then an empty list --
    // never a hardcoded guess).
    tags: (series.tags?.map((t) => t.display_label) ?? series.genre_names ?? []).slice(0, 2),
    imageUrl: series.poster_url ?? series.backdrop_url,
    isReal: true,
  }));
  const discoverCards = [
